...
public class StepFinishedCommand implements Command {
	...
	/**
	 * @param trieView
	 *            trie sobre el que se ha finalizado un paso
	 * @param animation
	 *            animationStep que ha finalizado de ejecutarse
	 * @param nextIndex
	 *            siguiente indice a ser ejecutado
	 * @param node
	 *            nodo que se proceso
	 * @param selectNextLetter
	 *            true si se debe seleccionar la siguente letra
	 * @param undo
	 *            true si el comando se ejecuta cuando se finaliza un paso
	...
	public void execute() {
		...
		if (controller.isInteractive()) {
			...
			controller.showLogMessage(TrieMessages.getInstance()
					.getMessageNextStep());
		}

		if ((!undo)
				&& (nextIndex >= trieView.getMaxAnimations() || nextIndex < 0)) {
			if (!controller.isInteractive())
