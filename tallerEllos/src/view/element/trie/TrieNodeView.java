...
public class TrieNodeView extends AbstractTrieNodeView implements TrieNodeListener<String>{
  ...
  public void valueIsWordChanged(TrieNodeEvent<String> event) {
    ...
    if (node.isWord()) {
      this.endWord = true;
      this.ptrDataView = new ChildPointerView(this, null, getShape().getPositionChildPtr(), DEF_LENGTH*2);
  ...
  public void moveBackwardFirstChild(double delta) {
    ...
    if ( posFinal.getX() <= firstChildFinalPos.getX() ) {
      ...
      if (!this.hasDataNode() || !getDataNode().isVisible()) {
        ptrChildView = new ChildPointerView(this, move, getShape().getPositionChildPtr(), DEF_LENGTH);
        this.firstChildPos = null;
      } else {
        this.firstChildPos = posFinal;
        ptrChildView = new DiagonalPointerView(this, move, getShape().getPositionChildPtr(), DEF_LENGTH); 
      }
      ...
      if (posChild.getX() > getShape().getPositionChildPtr().getX() ) {
        this.ptrChildView = new DiagonalPointerView(this, move, getShape().getPositionChildPtr(), DEF_LENGTH);
  ...
  public void moveForwardFirstChild(double delta) {
    ...
    PointerView ptr = new DiagonalPointerView(this, child, getShape().getPositionChildPtr(), DEF_LENGTH);
    distance = posFinal.distance( getShape().getPositionChildPtr() );
  ...
  public void addSibling(AbstractTrieNodeView node){
    ptrSiblView = new SiblingPointerView(this, node, getShape().getPositionSiblingPtr(), DEF_LENGTH);
  ...
  protected NodeShape createShape() {
    return new TrieNodeShape(getData(), getPosition(), getWidth(), getHeight(), DEF_FONT, DEF_STROKE);
  }
  ...
  private void removeChild() {
    ...
    if (node != null) {
      ...
      this.ptrChildView = new DiagonalPointerView(this, node, getShape().getPositionChildPtr(), DEF_LENGTH);
  ...
  private void drawPointer(Graphics2D g2, Point2D initPos, Point2D finPos, PointerView pointer) {
    ...
    if (pointer.isFlashing()) {
      color = Color.ORANGE;
      float DEF_DASH[] = { 0.5f };
      stroke = new BasicStroke(1.4f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, DEF_DASH, 0.0f);
    }
