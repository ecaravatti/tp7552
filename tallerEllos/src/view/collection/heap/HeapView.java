...
public class HeapView<T> extends AnimatedPanel implements HeapListener<T> {
	...
	private static final Font DEF_FONT_BALANCE = new Font("SansSerif",
			Font.PLAIN, 11);
	...
	private int nodePosY[] = new int[] { 100, 170, 170, 240, 240, 240, 240,
			310, 310, 310, 310, 310, 310, 310, 310, 380, 380, 380, 380, 380,
			380, 380, 380, 380, 380, 380, 380, 380, 380, 380, 380 };
	private int labelPosX[] = new int[] { 78, 103, 128, 153, 178, 203, 228,
			253, 278, 303, 328, 353, 378, 403, 428, 453, 478, 503, 528, 553,
			578, 603, 628, 653, 678, 703, 728, 753, 778, 803, 828 };
	...
	private void createHeap() {
		...
		Integer heapSize = Integer.valueOf(heapSizeComboBox.getSelectedItem()
				.toString());

		rectangles.add(createRect(50, 50, SQUARESIZE, SQUARESIZE, "ini"));
		for (int i = 1; i < heapSize + 1; i++) {
			rectangles.add(createRect(i * SQUARESIZE + 50, 50, SQUARESIZE,
					SQUARESIZE, ""));
		}
	...
	private BSTNodeShape createNode(String title) {
		BSTNodeShape node = new BSTNodeShape(title, null, DIAMETER, FONT,
	...
	private Arrow createArrow(Point2D parentPosition, Point2D currentPosition) {
		Point2D p1 = new Point2D.Double(parentPosition.getX() + DIAMETER / 2,
				parentPosition.getY() + DIAMETER / 2);
		Point2D p2 = new Point2D.Double(currentPosition.getX() + DIAMETER / 2,
	...
	private NodeShape createRect(int x, int y, int width, int height,
		...
		return new NodeShape(label, position, width, height, FONT, STROKE,
				false);
	}
	...
	private void initComponents() {
		...
		heapSizeComboBox.setModel(new javax.swing.DefaultComboBoxModel(
	...
	private void insertBtnActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_insertBtnActionPerformed
		...
		try {
			if (current == Integer.valueOf(
					heapSizeComboBox.getSelectedItem().toString()).intValue()) {
				return;
			}
