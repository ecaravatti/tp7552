...
public class TrieNode<Data> extends TrieNodeObservable<Data> {
   ...
   * @param character caracter que contiene el nodo
   * @param sibling hermano del nodo
   * @param child hijo del nodo
   */
  ...
  /**
   * Obtiene el hermano del nodo que contiene el caracter pasado como parametro.
   * @param chr caracter a buscar en uno de los hermano.
  ...
  /**
   * Agrega un hermano.
   * @param chr caracter que contendra el hermano
  ...
  public TrieNode<Data> addSibling(char chr) {
    ...
    for (node = this; node != null && node.getCharacter() <= chr; node = node.getSibling()) {
      if (node.getCharacter() != chr)
        node.fireSelectionModeChangedNotFound(node);
      left = node;
    }
  ...
  /**
   * @param chr caracter que debe contener el hijo buscado.
  ...
  /**
   * Agrega un hijo.
   * @param chr caracter que contendra el hijo
   ...
   * @param word true si en nodo finaliza una palabra, false en caso contrario.
   */
